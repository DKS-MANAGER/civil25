...
%% Part (b): Test the streamflow model
...
for sim = 1:n_simulations
    for day = 1:n_days
        if rand() < prob_rain
            rainfall_data_beta7(sim, day) = gamrnd_local(alpha, beta_new);
        else
...
% Save the figure to a PNG for embedding in reports
try
    saveas(gcf, 'sto4a_fixed_hist.png');
catch
    % ignore if figure not available (e.g., headless with no figure)
end
